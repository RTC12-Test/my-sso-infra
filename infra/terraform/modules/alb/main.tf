...
resource "aws_lb_listener" "https" {
  ...
  dynamic "default_action" {
    ...
    content {
      type             = var.aws_alb_routing_type
      target_group_arn = try(length(regex("green", aws_lb_target_group.alb_target_group[default_action.value.tg_name].name)) > 0, false) ? null : aws_lb_target_group.alb_target_group[default_action.value.tg_name].arn
